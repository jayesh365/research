...
from models.s4.s4 import S4Block as S4  # Can use full version instead of minimal S4D standalone below SSMKernelDiag
...
def train():
    ...
    for batch_idx, (inputs, targets) in pbar:
        ...
        print('\noutputs shape : ', outputs.shape, targets.shape)
        ...
        # print(len(A_matrices), A_matrices.keys())

        pbar.set_description(
            'Batch Idx: (%d/%d) | Loss: %.3f | Acc: %.3f%% (%d/%d)' %
            (batch_idx, len(trainloader), train_loss/(batch_idx+1), 100.*correct/total, correct, total)
        )
    # does_A_change(model, epoch)
...
# print(A_matrices.keys())


# diff_norm_A = torch.norm(A_matrices['epoch_0_layer_0'] - A_matrices['epoch_1_layer_0'], p='fro')

# print(f'A has changed {diff_norm_A} between epoch 0 and 1')
# wandb.finish()

#####

# print('\nTest dummy input and viz..\n')

# dummy_input = torch.randn(1, 1024, d_input).to(device)
